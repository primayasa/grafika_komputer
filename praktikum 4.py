...
def titik_berputar_custom(a, b, r, arah_putar, i):
    global sudut
    teta = sudut[i]/57.3
    x = r*math.cos(teta) + a
    y = r*math.sin(teta) + b
...
def lingkaran():
    ...
    circle = [[],[]]*360
    for i in range(360):
        circle[i] = [r * math.cos(i*3.14/180), r * math.sin(i*3.14/180)]
    draw_polyline(circle, 360, col)

def lingkaran_custom(a, b, red, green, blue):
    col = [red, green, blue]
    r = 50.0
    circle = [[],[]]*360
    for i in range(360):
        circle[i] = [r * math.cos(i*3.14/180) + a, r * math.sin(i*3.14/180) + b]
    draw_polyline(circle, 360, col)
...
def draw0():
    ...
    # ===== Pertemuan 4 =====
    lingkaran()
    lingkaran_custom(100, 100, 0.0, 1.0, 0.0)
    lingkaran_custom(-100, 100, 0.0, 1.0, 0.0)
    lingkaran_custom(-100, -100, 0.0, 1.0, 0.0)
    lingkaran_custom(100, -100, 0.0, 1.0, 0.0)

    titik_berputar(50, 0)
    titik_berputar_custom(100, 100, 50, 1, 1)
    titik_berputar_custom(-100, 100, 50, 5, 2)
    titik_berputar_custom(-100, -100, 50, 4, 3)
    titik_berputar_custom(100, -100, 50, 10, 4)
